...
#SBATCH --job-name=verl-fully-async
#SBATCH --nodes=4
...
#SBATCH --output=./logs_async/slurm-%j.out
#SBATCH --error=./logs_async/slurm-%j.err
#SBATCH --nodelist=useocpm2m-097-[008,017,032,014,015,024,026,046,049,050,028,030,038,039,075,076,080,082]
...
# Fully async policy requires vLLM V1 engine
...
##########################################################################
### Algorithm parameters (GRPO with KL loss, from original)
##########################################################################
adv_estimator=grpo
...
##########################################################################
### Response length parameters (kept from original)
##########################################################################
max_prompt_length=1024
max_response_length=32768

##########################################################################
### Sampling parameters (from dapo_30b reference)
##########################################################################
temperature=1.0
top_p=1.0
top_k=-1
...
##########################################################################
### Performance parameters (from dapo_30b reference)
##########################################################################
sp_size=4
...
##########################################################################
### Fully Async Policy: Resource Split Configuration
### Async: 1 node for Trainer + 3 nodes for Rollout (resource isolation)
##########################################################################
NNODES_TRAIN=1
NNODES_ROLLOUT=3
NGPUS_PER_NODE=8
...
##########################################################################
### Fully Async Policy: Async Training Parameters
### (derived from dapo_30b reference, adapted to GSM8K data volume)
##########################################################################
train_bsz=128
train_prompt_bsz=0
gen_prompt_bsz=1
...
# total_rollout_steps = train_batch_size * num_steps (64 * 120 ≈ 1 epoch of GSM8K)
total_rollout_steps=$((train_bsz * 120))
test_freq=5
...
echo "Start to train..."
echo "  Fully Async Policy Mode"
echo "  Trainer nodes: ${NNODES_TRAIN}, Rollout nodes: ${NNODES_ROLLOUT}"
echo "  Total rollout steps: ${total_rollout_steps}"
echo "  Staleness threshold: ${staleness_threshold}"
echo "  Trigger param sync step: ${trigger_parameter_sync_step}"
echo "  PPO mini batch size: ${train_prompt_mini_bsz}"
